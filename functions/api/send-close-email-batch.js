...
export async function onRequest({ request, env }) {
  ...
  const limit = Math.max(1, Math.min(45, Number(url.searchParams.get('limit') || '25')));

  const rs = await env.DB.prepare(
    "SELECT DISTINCT TRIM(email) AS email FROM users WHERE email IS NOT NULL AND TRIM(email) != '' ORDER BY created_at DESC LIMIT ? OFFSET ?"
